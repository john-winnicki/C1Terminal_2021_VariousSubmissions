...
class AlgoStrategy(gamelib.AlgoCore):
	...
	def on_game_start(self, config):
		...
		self.path2 = [[10,9],[11,9],[10,10],[11,10],[13,3],[14,3]]
		self.path3 = [[16,9],[17,9],[16,10],[17,10],[13,3],[14,3]]
		...
		wall_locations_2 = [[0, 13], [1, 13], [2, 13], [3, 13], [24, 13], [25, 13], [26, 13], [27, 13], [1, 12], [2, 12], [3, 12], [24, 12], [25, 12], [26, 12], [3, 11], [24, 11], [3, 11], [4, 11], [5, 11], [22, 11], [23, 11], [24, 11], [5, 10], [22, 10], [6,10],[21,10],[7,10],[20,10],[8,10],[19,10],[9,10],[18,10],[10,10],[17,10],[11,10],[16,10],[12,10],[15,10],[13,10],[14,10]]
		...
		support_locations_2 = [[13,3],[14,3],[15,4],[12,4],[16,5]]
		support_locations_3 = [[11,5],[17,6],[10,6],[18,7],[9,7],[19,8],[8,8]]
	...
	def hard_scoutStrat_part1(self, game_state):
		...
		turretCount1 = self.detect_enemy_unit(game_state, unit_type=TURRET, valid_x=[0,3], valid_y=[14, 17])
		turretCount2 = self.detect_enemy_unit(game_state, unit_type=TURRET, valid_x=[6,13], valid_y=[14, 27])
		turretCount3 = self.detect_enemy_unit(game_state, unit_type=TURRET, valid_x=[14,21], valid_y=[14, 27])
		turretCount4 = self.detect_enemy_unit(game_state, unit_type=TURRET, valid_x=[24,27], valid_y=[14, 21])

		gamelib.debug_write('turretCount1: {}'.format(turretCount1))
		gamelib.debug_write('turretCount2: {}'.format(turretCount2))
		gamelib.debug_write('turretCount3: {}'.format(turretCount3))
		gamelib.debug_write('turretCount4: {}'.format(turretCount4))

		unitCount2 = self.detect_enemy_unit(game_state, valid_x=[6,13], valid_y=[14, 27])
		unitCount3 = self.detect_enemy_unit(game_state, valid_x=[14,21], valid_y=[14, 27])

		num = 1
		if turretCount2<=1 and unitCount2<9:
			num = 2
		elif turretCount3<=1 and unitCount3<9:
			num = 2
		elif turretCount1<turretCount4:
			num = 1
		else:
			num = 4
	...
	def hard_scoutStrat_part2(self, game_state):
		...
		if self.first_time:
			...
			if game_state.my_health <= 25:
				self.first_time = True
				self.open_channel = 5;
			else: self.first_time = False
	...
	def build_reactive_defense(self, game_state):
		...
		for location in self.scored_on_locations:
			badX.append(location[0])
			if location[0]>0:
				badX.append(location[0]-1)
			if location[0]<27:
				badX.append(location[0]+1)
		for loc in self.frame:
			if loc[0] in badX:
				game_state.attempt_upgrade(loc)
