...
class AlgoStrategy(gamelib.AlgoCore):
    ...
    def starter_strategy(self, game_state):
        ...
        """
        if game_state.turn_number <= 15 and game_state.turn_number%3!=0 and (
        ...
        elif game_state.turn_number<=15: self.build_defences(game_state)
    ...
    def hard_scoutStrat_left(self, game_state):
        ...
        elif game_state.get_resource(MP, 0) >= 10:
            # mp = game_state.get_resource(MP, 0)
            # game_state.attempt_spawn(DEMOLISHER, [24,10],int(mp//10))
            # game_state.attempt_spawn(INTERCEPTOR, [22,8],int(mp//10))
            mp = game_state.get_resource(MP, 0)
            game_state.attempt_spawn(SCOUT, [11, 2], math.floor(mp / 4))
            game_state.attempt_spawn(SCOUT, [16, 2], math.floor(mp / 2))
