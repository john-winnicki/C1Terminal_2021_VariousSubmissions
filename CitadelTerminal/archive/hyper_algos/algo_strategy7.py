...
class AlgoStrategy(gamelib.AlgoCore):
	...
	def on_game_start(self, config):
		...
		support_locations_3 = [[12,4],[11,5],[10,5],[10,6],[9,7],[8,8]]
	...
	def hard_scoutStrat_part2(self, game_state):
		...
		if self.first_time:
			...
			if self.open_channel==1:
				game_state.attempt_spawn(DEMOLISHER, [11,2], 4)
				# game_state.attempt_spawn(SCOUT, [15,1], 4)
				game_state.attempt_spawn(SCOUT, [24,10], 1000)
			elif self.open_channel==4:
				game_state.attempt_spawn(DEMOLISHER, [16,2], 4)
				# game_state.attempt_spawn(SCOUT, [12,1], 4)
				game_state.attempt_spawn(SCOUT, [3,10], 1000)
			else:
				game_state.attempt_spawn(DEMOLISHER, [14,0], 4)
				# game_state.attempt_spawn(SCOUT, [13,1],5)
				game_state.attempt_spawn(SCOUT, [8,15],1000)
			...
			if self.open_channel==1:
				game_state.attempt_spawn(SCOUT, [11,2], 2)
				game_state.attempt_spawn(SCOUT, [15,1], 1000)
			elif self.open_channel==4:
				game_state.attempt_spawn(SCOUT, [15,1], 2)
				game_state.attempt_spawn(SCOUT, [13,0], 1000)
			else:
				game_state.attempt_spawn(SCOUT, [14,0], 2)
	...
	def build_defences(self, game_state):
		...
		# 	if not game_state.can_spawn(TURRET, loc):
		# 		count += 1
		# for waller in wall_locations_1:
		# 	if not game_state.can_spawn(WALL, waller):
		# 		count += 1
		# for loc in turret_locations_3:
		# 	if not game_state.can_spawn(TURRET, loc):
		# 		count +=1 
		# for loc in support_locations_1:
		# 	if not game_state.can_spawn(SUPPORT, loc):
		# 		count += 1
