...
class AlgoStrategy(gamelib.AlgoCore):
	...
	def on_game_start(self, config):
		...
		support_locations_2 = [[13,3],[14,2],[14,3],[15,4],[16,5],[17,6],[18,7],[19,8], [13,3],[14,3],[13,4],[14,4]]
	...
	def hard_scoutStrat_part1(self, game_state):
		...
		count4 = self.detect_enemy_unit(game_state, unit_type=TURRET, valid_x=[24,27], valid_y=[14, 17])
	...
	def hard_scoutStrat_part2(self, game_state):
		...
		if self.first_time:
			...
			if self.open_channel==1:
				...
				game_state.attempt_spawn(SCOUT, [8,15],10)
			...
			if self.open_channel==1:
				game_state.attempt_spawn(SCOUT, [11,2], 10)
				game_state.attempt_spawn(SCOUT, [15,1], 10)
			elif self.open_channel==4:
				game_state.attempt_spawn(SCOUT, [15,1], 10)
	...
	def soft_scoutStrat(self, game_state):
		...
		if (num==0 and game_state.get_resource(MP, 0) >= 5):
			game_state.attempt_spawn(SCOUT, [14,0], 10)
			willNotAttack = False
		if (num==1 and game_state.get_resource(MP, 0) >= 5):
			game_state.attempt_spawn(SCOUT, [13,0], 10)
			willNotAttack = False
