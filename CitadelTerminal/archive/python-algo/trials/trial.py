...
class AlgoStrategy(gamelib.AlgoCore):
    ...
    def find_deficit(self, game_state):
        ...
        exclude_in_right = []  # stores points that have paths going through them, so we can ignore future paths that end up being basically the same
        exclude_in_left = []

        excluded = 0

        for h in enemy_right_weak:
            p = (self.simulate_unit_journey(game_state, h, edge="SW",
                                            exclude=exclude_in_right), "SW")  # they target the opposite edge
            if p[0] is not None:
                # add all the points on the path to exclusions
                exclude_in_right += p[0][1]
        ...
        for h in enemy_left_weak:
            p = (self.simulate_unit_journey(game_state,
                                            h, edge="SE", exclude=exclude_in_left), "SE")
            if p[0] is not None:
                # add all the points on the path to exclusions
                exclude_in_left += p[0][1]
        ...
        res = game_state.get_resource(SP, 0)
        ...
        while len(possibilities) != 0:
            ...
            for path in possibilities:
                ...
                if game_state.get_resource(SP, 0) < 1:
    ...
    def max_onslaught(self, game_state):
        res = game_state.get_resource(MP, 1)
        return (res / 2) * 15
    ...
    def remove_front_defences(self, game_state):
        locs = [[13, 11], [14, 11]]
        if self.enemy_has_frontline(game_state) and game_state.get_resource(MP, 0) >= 5:
            game_state.attempt_spawn(DEMOLISHER, locs[0])
            game_state.attempt_spawn(INTERCEPTOR, locs[1])

        else:
            self.build_defences(game_state)

    def enemy_has_frontline(self, game_state):
        locs = []
        for x in range(0, 28):
            if game_state.contains_stationary_unit([x, 14]):
                locs.append([x, 14])
        return True if len(locs) >= 14 else False

    def hard_scoutStrat_part1(self, game_state):
        if game_state.get_resource(MP, 0) >= 20:
            clear_path = [[27, 13], [26, 13], [
                26, 12], [25, 12], [24, 12], [24, 11]]
    ...
    def hard_scoutStrat_part2(self, game_state):
        if game_state.get_resource(MP, 0) >= 20:
            ...
            mp = game_state.get_resource(MP, 0)
            if (int(mp) - 1 > 0):
                game_state.attempt_spawn(SCOUT, [13, 0], int(mp) - 1)
        else:
            self.build_defences(game_state)
    ...
    def build_defences(self, game_state):
        ...
        # TODO: Add [22,10]  and [5,10] to wl1
        turret_locations_1 = [[2, 11], [25, 11]]
        wall_locations_1 = [[0, 13], [1, 13], [2, 13], [3, 13], [3, 12], [
            13, 10], [14, 10], [24, 12], [24, 13], [25, 13], [26, 13], [27, 13]]
        ...
        support_locations_2 = [[13, 3], [14, 2], [14, 3], [15, 4], [
            17, 5], [18, 6], [19, 7], [16, 5], [17, 6], [18, 7]]
