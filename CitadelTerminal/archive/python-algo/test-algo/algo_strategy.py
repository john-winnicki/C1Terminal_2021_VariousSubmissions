...
class AlgoStrategy(gamelib.AlgoCore):
    ...
    def on_game_start(self, config):
        ...
        self.config = config
        global WALL, SUPPORT, TURRET, SCOUT, DEMOLISHER, INTERCEPTOR, MP, SP, MP_REM, SP_REM, HP_REM
        global WALL_COST, SUPPORT_COST, TURRET_COST
        global SCOUT_COST, DEMOLISHER_COST, INTERCEPTOR_COST
        ...
        # Initialize the costs of each unit
        WALL_COST = 1
        SUPPORT_COST = 4
        TURRET_COST = 2
        SCOUT_COST = 1
    ...
    def on_turn(self, turn_state):
        ...
        if game_state.turn_number == 0:
            self.john_code(game_state)
            # self.initial_upgrade(game_state)
        else:
            self.john_code(game_state)
            self.reactive_defences(game_state)
            # self.initial_upgrade(game_state)

        game_state.submit_turn()

    def initial_setup(self, game_state):
        global wall_locations, support_locations, turret_locations
        wall_locations = [[0, 13], [2, 13], [3, 13], [24, 3], [25, 3], [
            26, 3], [27, 3], [5, 10], [13, 10], [14, 10], [22, 10]]

        support_locations = [[14, 2]]

        turret_locations = [[2, 11], [25, 11], [14, 9]]

        # Placing walls
        for each in wall_locations:
            game_state.attempt_spawn(WALL, each)

        # Placing Supports
        for each in support_locations:
            game_state.attempt_spawn(SUPPORT, each)

        # Placing turrets
        for each in turret_locations:
            game_state.attempt_spawn(TURRET, each)

    def initial_upgrade(self, game_state):
        game_state.attempt_upgrade(turret_locations)
        game_state.attempt_upgrade(wall_locations)
        game_state.attempt_upgrade(support_locations)

    def reactive_defences(self, game_state):
...
# *********************************
    ...
    def john_code(self, game_state):
        turret_locations_1 = [[2, 11], [25, 11]]
        wall_locations_1 = [[0, 13], [1, 13], [2, 13], [3, 13], [3, 12], [5, 10], [
            13, 10], [14, 10], [22, 10], [24, 12], [24, 13], [25, 13], [26, 13], [27, 13]]
        wall_locations_2 = [[0, 13], [1, 13], [2, 13], [3, 13], [24, 13], [25, 13], [26, 13], [27, 13], [1, 12], [2, 12], [3, 12], [4, 12], [23, 12], [24, 12], [25, 12], [26, 12], [3, 11], [4, 11], [
            5, 11], [22, 11], [23, 11], [24, 11], [5, 10], [7, 10], [8, 10], [9, 10], [11, 10], [12, 10], [13, 10], [14, 10], [15, 10], [16, 10], [17, 10], [18, 10], [19, 10], [20, 10], [22, 10]]
        support_locations_1 = [[13, 2]]
        turret_locations_3 = [[13, 9]]
        turret_locations_2 = [[8, 9], [19, 9]]
        wall_locations_3 = [[6, 9], [7, 9], [9, 9], [10, 9], [11, 9], [12, 9], [
            14, 9], [15, 9], [16, 9], [17, 9], [18, 9], [20, 9], [21, 9]]
        support_locations_2 = [[13, 3], [14, 2], [14, 3]]
        ...
        game_state.attempt_spawn(SUPPORT, support_locations_1)
