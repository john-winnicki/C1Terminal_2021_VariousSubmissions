...
class AlgoStrategy(gamelib.AlgoCore):
	...
	def starter_strategy(self, game_state):
		...
		#     self.build_reactive_defense(game_state)
			...
			if game_state.get_resource(MP, 0) >= 18 and self.open_channel==0:
	...
	def hard_scoutStrat_part2(self, game_state):
		...
		if game_state.get_resource(MP, 0) >= 20: #and self.doesNotSelfDestruct(game_state,[13,0]) and self.doesNotSelfDestruct(game_state,[7,6]):
			# mp = game_state.get_resource(MP, 0)
			game_state.attempt_spawn(DEMOLISHER, [24,10],int(mp//5))
			# game_state.attempt_spawn(INTERCEPTOR, [22,8],int(mp//10))
			mp = game_state.get_resource(MP, 0)
			game_state.attempt_spawn(SCOUT,[13,0],int(math.floor(mp/4)))
			mp = game_state.get_resource(MP, 0)
	...
	def soft_scoutStrat(self, game_state):
		willNotAttack = True
		if (game_state.get_resource(MP, 0) >= 5) and self.doesNotSelfDestruct(game_state,[14,0]):
			game_state.attempt_spawn(SCOUT, [14,0], 10)
			willNotAttack = False
		if (game_state.get_resource(MP, 0) >= 5) and self.doesNotSelfDestruct(game_state,[13,0]):
