...
class AlgoStrategy(gamelib.AlgoCore):
	...
	def starter_strategy(self, game_state):
		...
		"""
		if game_state.turn_number<=15 and (self.doesNotSelfDestruct(game_state, [13,0]) or self.doesNotSelfDestruct(game_state, [14,0])):
		...
		#     self.build_reactive_defense(game_state)
			...
			elif game_state.get_resource(MP, 0) >= 18 and self.open_channel==0:
	...
	def hard_scoutStrat_part1(self, game_state):
		if game_state.get_resource(MP, 0) >= 20:
			...
			clear_path = [[15,1],[15,2],[16,2],[16,3],[17,3],[17,4],[18,4],[18,5],[19,5],[19,6],[20,6],
			[20,7],[21,7],[21,8],[22,8],[22,9],[23,9],[23,10],[24,10],[24,11],[24,12],[25,12],[26,12],
			[26,13],[27,13]]
			for loc in clear_path:
				game_state.attempt_remove(loc)
			self.open_channel = 1
	...
	def hard_scoutStrat_part2(self, game_state):
		...
		orig_health = game_state.enemy_health

		mp = game_state.get_resource(MP, 0)
		if game_state.get_resource(MP, 0) >= 50:
			game_state.attempt_spawn(SCOUT,[13+num,0],int(math.floor(mp)))
		if game_state.get_resource(MP, 0) >= 20: #and self.doesNotSelfDestruct(game_state,[13,0]) and self.doesNotSelfDestruct(game_state,[7,6]):
			# mp = game_state.get_resource(MP, 0)
			game_state.attempt_spawn(DEMOLISHER, [24,10],int(mp//10))
			game_state.attempt_spawn(DEMOLISHER, [23,10],int(mp//10))
			# game_state.attempt_spawn(INTERCEPTOR, [22,8],int(mp//10))
			mp = game_state.get_resource(MP, 0)
			game_state.attempt_spawn(SCOUT,[13+num,0],int(math.floor(mp/4)))
			mp = game_state.get_resource(MP, 0)
		...
		# if orig_health != game_state.enemy_health:
		self.open_channel = 0;
